...
\begin{abstract}
We study a cross-silo federated learning (FL) setting for robotic
manipulation in which a central server coordinates multiple
clients, each client owns private demonstration trajectories, and
only model updates are communicated. In FLAME~\cite{flame2025},
standard FedAvg-style training~\cite{fedavg} with a CNN policy
collapses on contact-rich tasks such as \emph{Insert Peg} and
\emph{Scoop}. We argue that this failure mixes two distinct
sources of difficulty. At the task level, contact manipulation
requires metric 3-D structure that is weakly observed from a
single front RGB image. At the federated level, client $c$ observes
the same robot workspace through client-specific camera intrinsics
$K_c$ and camera-to-robot extrinsics $T_c$, so a shared 2-D encoder
must absorb both manipulation semantics and camera-dependent
projection geometry. We propose a camera-aware federated policy
framework that separates these axes. Each client keeps raw images,
$K_c$, $T_c$, depth maps, and voxel features local; it communicates
only lightweight trainable modules such as LoRA adapters, voxel
fusion layers, and the action decoder. A frozen DINO-style visual
backbone~\cite{dinov2}, a frozen Depth-Anything depth
prior~\cite{depthanything}, and a VolumeDP-inspired
world-frame lift~\cite{volumedp2026} convert private
client-frame observations into an aligned representation before
server aggregation. We further include a replay-ceiling diagnostic
to fix the action interface before comparing representation and FL
methods. The resulting report specifies the data distribution,
heterogeneity model, architecture, aggregation scope, and metrics
needed to evaluate whether camera-aware representation alignment
improves mean and worst-client success rate under federated
deployment.
\end{abstract}
...
Tab.~\ref{tab:flame} shows the canonical CNN+MLP+FedAvg
\cite{fedavg} recipe
...
Fig.~\ref{fig:framework} and Tab.~\ref{tab:setting} summarise the
setting before we introduce any proposed component. This is a
server-mediated FL problem, not centralized training: the server
selects clients and aggregates model updates, while each client
retains its own demonstrations, camera calibration, raw RGB/depth
observations, and rollout environment. The policy input is a
front-view observation, proprioception, and client-local camera
metadata; the output is a chunked robot action executed through a
fixed controller. Heterogeneity is induced by environment/task
variation and, in the experiments targeted by this proposal, by
client-specific camera projections.
...
\begin{tikzpicture}[
  ...
  client/.style={box,fill=blue!5,minimum width=4.0cm},
  local/.style={box,fill=gray!12,minimum width=4.0cm},
  trainable/.style={box,fill=blue!14,minimum width=4.0cm},
  server/.style={box,fill=orange!18,thick,minimum width=4.0cm},
  arr/.style={-{Stealth},semithick},
  comm/.style={-{Stealth},thick,blue!65!black}
]
\node[client] (c1) {\textbf{Client $c$}\\private dataset
$\mathcal{D}_c=\{(o_t^c,a_t^c)\}$};
\node[local,below=3pt of c1] (obs) {Observation $o_t^c$: RGB image,
proprioception, local calibration $(K_c,T_c)$};
\node[local,below=3pt of obs] (lift) {Local-only processing:
depth prior + world-frame voxel lift};
\node[trainable,below=3pt of lift] (pol) {Trainable surface:
LoRA adapters, voxel fusion, action decoder};
\node[server,right=1.3cm of obs] (srv) {\textbf{Server}\\client
sampling + FedAvg/FedProx/SCAFFOLD aggregation};
\node[local,below=3pt of srv] (global) {Global / partially shared
policy parameters $\theta$};
\node[client,right=1.3cm of srv] (eval) {\textbf{Evaluation}\\mean
SR, worst-client SR, variance, communication};
\draw[arr] (c1) -- (obs);
\draw[arr] (obs) -- (lift);
\draw[arr] (lift) -- (pol);
\draw[comm] (pol.east) -- node[above,align=center]{model update only\\no raw data,
no $(K_c,T_c)$} (srv.west);
\draw[comm] (srv.south) -- (global.north);
\draw[comm] (global.west) -| node[pos=0.25,below]{broadcast $\theta$} (pol.east);
\draw[arr] (srv) -- (eval);
\end{tikzpicture}
}
\caption{Framework overview. Each client stores demonstrations and
camera metadata locally. The proposed camera-aware policy uses
client-local calibration to align observations before communication;
the server receives only trainable model updates. This figure fixes
the problem setting for all tables and ablations in the report.}
\label{fig:framework}
\end{figure*}
...
\centering
\small
\caption{Problem setting and method comparison. The existing FLAME
baseline and our proposed framework operate in the same
server--client FL protocol, but they differ in the representation
sent to aggregation and in how camera heterogeneity is handled.}
\label{tab:setting}
\begin{tabular}{p{2.5cm}p{5.1cm}p{5.1cm}}
\toprule
Aspect & Existing baseline & Proposed framework\\
\midrule
FL protocol & Server-mediated cross-silo FL; clients train locally
and the server aggregates updates with FedAvg-class methods
\cite{fedavg}. & Same protocol; the contribution is a different
client representation and aggregation surface, not a different
privacy model.\\
Tasks & Four RLBench / FLAME tasks: \emph{Slide Block},
\emph{Close Box}, \emph{Insert Peg}, and \emph{Scoop}. &
Same task suite; we separate single-task same-env, single-task
multi-env, and later multi-task evaluation.\\
Client data & Demonstration trajectories for one or more RLBench
tasks. Each sample contains observation $o_t^c$ and expert action
$a_t^c$. Data are partitioned by client/environment and raw
trajectories do not leave the client. & Same trajectories, with
explicit use of client-local camera intrinsics $K_c$ and extrinsics
$T_c$.\\
Input / output & Front RGB + proprioception $\rightarrow$ robot
action. & Front RGB + proprioception + local calibration
$\rightarrow$ chunked action through a fixed JP-servo interface.\\
Heterogeneity & Non-IID environment/task data and camera-dependent
2-D observations are both absorbed by a shared CNN encoder. &
Task/environment heterogeneity remains; camera projection
heterogeneity is reduced by local world-frame lifting before
aggregation.\\
Backbone & CNN + MLP / diffusion head trained from local data. &
Frozen DINO-style ViT~\cite{dinov2} + frozen depth prior
\cite{depthanything}; only lightweight adapters and heads are
trainable.\\
Aggregated surface & Usually all trainable policy weights. &
LoRA adapters, voxel-fusion / token modules, and optionally the
decoder; camera parameters, depth maps, and voxels remain local.\\
Metrics & Mean task success rate (SR). & Mean SR, worst-client SR,
client variance, communication per round, and same-env to
multi-env gap.\\
\bottomrule
\end{tabular}
\end{table*}
...
We consider a set of clients $\mathcal{C}=\{1,\ldots,C\}$ coordinated
by one server. Client $c\in\mathcal{C}$ owns a private dataset
$\mathcal{D}_c=\{(o_i^c,a_i^c)\}_{i=1}^{n_c}$ of demonstration
pairs. The observation $o_i^c$ contains a front RGB image
$I_i^c$, proprioceptive state $s_i^c$, and client-local camera
calibration. The action label $a_i^c$ is a robot action chunk
executed through a fixed controller. The camera calibration is
written as $(K_c,T_c)$, where $K_c$ is the camera intrinsic matrix
and $T_c$ is the camera-to-robot-base extrinsic transform. These
calibration variables are observed by the client but are not sent
to the server.
...
\begin{equation}
  ...
  = \sum_{c\in\mathcal{C}} p_c
    \mathbb{E}_{(o,a)\sim\mathcal{D}_c}
    \left[\ell(f_{\theta}(o),a)\right],
  \label{eq:fl-objective}
\end{equation}
where $\theta$ denotes the server-shared policy parameters,
$p_c=n_c/\sum_{j}n_j$ is the client weight, $f_{\theta}$ is the
policy, and $\ell$ is the imitation-learning loss. In FedAvg
\cite{fedavg}, the server broadcasts $\theta^r$ at round $r$,
clients perform local optimisation on $\mathcal{D}_c$, and the
server averages the resulting updates. FedProx~\cite{fedprox}
adds a proximal regulariser to each client's local objective, while
SCAFFOLD~\cite{scaffold} uses control variates to reduce client
drift.

In this proposal, heterogeneity has two components. Data
heterogeneity means that clients may differ in task, object state,
background, demonstration style, or controller timing. Camera
heterogeneity means that clients may share the same task semantics
but observe them through different $(K_c,T_c)$, so the same
world-frame point projects to different pixels. The proposed
method does not remove all non-IID data effects; it specifically
targets the camera-induced component by aligning client features in
a local world frame before aggregation.
...
workspace calibration) to build a paper-close approximation to
VolumeDP \cite{volumedp2026}. Concretely, the client forms a
two-frame RGB pair $(t,t\!-\!1)$, encodes each frame with frozen
DINOv3 patch features and a frozen Depth-Anything depth prior,
projects those features into a predefined world-aligned voxel grid
using $(K_c, T_c)$, averages the per-frame voxel features, and
adds voxel-centre positional encodings. A goal-conditioned softmax
token learner over voxels then produces a set of spatial tokens;
to better match Fig.~2 of VolumeDP, we append a learned proxy
goal token to the decoder token set and supervise voxel weights
with an end-effector-centred auxiliary BCE mask.

\paragraph{VolumeDP fidelity note.}
This should be described as a \emph{paper-close approximation},
not as an exact reproduction of VolumeDP. Relative to Fig.~2, the
current implementation matches the world-frame voxel
representation, temporal two-frame input, spatial-token decoder
interface, and end-effector-centred auxiliary supervision.
However, it still differs from the paper in four material ways:
we keep feature sampling instead of deformable volume--image
cross-attention, use a learned proxy goal token rather than a real
language instruction token, omit gripper-transition auxiliary
supervision, and run a smaller configuration ($24^3$ grid, 200
tokens, 192-d token width) than the paper's full $40^3$/512-d
setting. We make this distinction explicit because the negative
same-env probes below should not be over-interpreted as a negative
result against the full VolumeDP design.
...
Both ideas feed into diffusion decoders \cite{diffpolicy}, but we
do \emph{not} force the same conditioning layout for every probe.
For the strongest 2-D baseline of Tab.~\ref{tab:slide-ablation},
we use a compact 20-step DDPM conditioned on a pooled image
embedding with gated-FiLM proprio fusion. For the paper-close
VolumeDP runs, we instead use a 100-step DDPM conditioned on the
spatial-token set, an appended proxy goal token, and a separate
proprio token, which is closer to Fig.~2 of VolumeDP. A separate
sigmoid+BCE gripper head handles the binary gripper event. For the
FL-facing comparisons we keep JP-servo $g\!=\!20$ from the
diagnostic of \S\ref{sec:diagnostic}; the slide same-env
paper-close sanity run is kept on the strongest currently stable
JV-direct setup so that the representation change is isolated.
...
\node[trainable,below=2pt of lor] (vol) {Paper-close VolumeDP (voxel lift + soft token learner)};
\node[trainable,below=2pt of vol] (dif) {Diffusion decoder};
...
adapters, the voxel-lift / spatial-token module, and the diffusion decoder
...
MLPs, the voxel-fusion and spatial-token learner weights, and (in
the standard variant) the diffusion decoder.
...
token-learning block comes from Idea~B, and the diffusion decoder
is shared by both. We compare four variants that
correspond to different combinations of \emph{what stays local}.

\noindent\textbf{V1 Global lightweight (baseline).} Aggregate
\{LoRA, fusion projector, voxel lift, spatial-token learner,
...
head as the baseline. These were \emph{early coarse probes}, not
yet the paper-close VolumeDP setting: they used single-frame
input, low token budgets, a smaller grid, no decoder-side goal
token, and no auxiliary voxel supervision. Volume bounds and grid
resolution are the two free parameters of this early volumetric
module; we sweep three representative settings
(Tab.~\ref{tab:volumedp-sameenv}). All runs are 50 epochs on
\texttt{env0}, 20 evaluation episodes, JV direct.

\begin{table}[h]
\centering
\footnotesize
\caption{Early same-env volumetric probe (coarse Idea~B
approximation, no $(K_c, T_c)$ shift across demos). w1: default workspace volume
...
\paragraph{Paper-close follow-up.}
We do \emph{not} treat Tab.~\ref{tab:volumedp-sameenv} as a fair
test of VolumeDP itself. The next run before federated evaluation
is a paper-close same-env approximation on \emph{Slide} that adds
(i)~temporal two-frame RGB input, (ii)~a $24^3$ voxel grid,
(iii)~200 spatial tokens, (iv)~a 100-step diffusion decoder,
(v)~a decoder-side proxy goal token, and (vi)~end-effector-centred
auxiliary voxel supervision. This follow-up is designed to answer
a narrower question: whether the early negative same-env result was
caused by the world-frame inductive bias itself, or by an
under-specified approximation of Fig.~2.
...
\textbf{P2b Paper-close VolumeDP sanity (wk 4).} Before claiming a
negative result for Idea~B on same-env data, re-run the single-env
probe with a Figure-2-aligned approximation: temporal RGB pairs,
$24^3$ voxels, 200 spatial tokens, a 100-step decoder, a
decoder-side proxy goal token, and end-effector auxiliary
supervision. This isolates whether the regression in
Tab.~\ref{tab:volumedp-sameenv} came from the world-frame
representation itself or from an under-powered approximation of
VolumeDP.
