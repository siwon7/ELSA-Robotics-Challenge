...
RESULT_ROOT="$ARTIFACT_ROOT/results/balanced_sameenv_search_20260508"
CKPT_ROOT="$ARTIFACT_ROOT/model_checkpoints/balanced_sameenv_search_20260508"
LOG_ROOT="$ARTIFACT_ROOT/logs/balanced_sameenv_search_20260508"
SESSION_NAME="${BALANCED_SEARCH_SESSION:-balanced_sameenv_search_20260508}"
WAITER_SESSION="${BALANCED_SEARCH_WAITER_SESSION:-balanced_sameenv_search_wait_20260508}"
EPOCHS="${BALANCED_SEARCH_EPOCHS:-100}"
...
run_train() {
  ...
  echo "=== TRAIN START gpu=$gpu task=$task run=$run_name cfg=$cfg epochs=$EPOCHS $(date '+%F %T') ==="
  export CUDA_VISIBLE_DEVICES="$gpu"
  elsa_run_with_cpu_limit "$gpu" "${cmd[@]}"
  echo "=== TRAIN END gpu=$gpu task=$task run=$run_name $(date '+%F %T') ==="
}
  ...
  case "$gpu" in
    0)
      run_train 0 slide_block_to_target \
        ...
        slide_jprel_w4_direct_grid16_eeaux_gripw6xk8_e${EPOCHS}_s0
      ;;
    1)
      run_train 1 close_box \
        experiments/sameenv_volumedp_full_dinov3_depth_lora8_jprel_w4_direct_grid16_eeaux_gripw6xk8.yaml \
        close_jprel_w4_direct_grid16_eeaux_gripw6xk8_e${EPOCHS}_s0
      ;;
    2)
      run_train 2 insert_onto_square_peg \
        ...
        insert_jprel_w4_jvservo_grid16_eeaux_gripw6xk8_e${EPOCHS}_s0
      ;;
    3)
      run_train 3 scoop_with_spatula \
        experiments/sameenv_volumedp_full_dinov3_depth_lora8_jprel_w4_jvservo_grid16_eeaux_gripw6xk8.yaml \
        scoop_jprel_w4_jvservo_grid16_eeaux_gripw6xk8_e${EPOCHS}_s0
      ;;
    *)
...
waiter() {
  ...
  exec > >(tee -a "$log") 2>&1
  echo "=== BALANCED SEARCH WAITER START $(date '+%F %T') ==="
  ...
  echo "[$(date '+%F %T')] blockers clear; launching balanced search"
  launch_run_session
}
